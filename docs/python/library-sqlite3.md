...
* Reference describes the classes and functions this module
defines.

* How-to guides details how to handle specific tasks.

* Explanation provides in-depth background on
transaction control.

See also

https://www.sqlite.orgThe SQLite web page; the documentation describes the syntax and the
...
The result is a `list` of two `tuple`s, one per row,
...
* factory (Connection) – A custom subclass of `Connection` to create the connection with,
...
* uri (bool) – If set to `True`, database is interpreted as a
...
Changed in version 3.13: Positional use of the parameters timeout, detect_types,
...
Set `autocommit` to this constant to select
...
https://www.sqlite.org/c3ref/c_dbconfig_defensive.htmlSQLite docs: Database Connection Configuration Options
...
Changed in version 3.13: A `ResourceWarning` is emitted if `close()` is not called before
...
Open a `Blob` handle to an existing
BLOB.
...
* readonly (bool) – Set to `True` if the blob should be opened without write
permissions.
Defaults to `False`.
...
The blob size cannot be changed using the `Blob` class.
...
If `autocommit` is `True` or `LEGACY_TRANSACTION_CONTROL`,
no implicit transaction control is executed.
Make sure to `commit()` before closing
to avoid losing pending changes.

execute(sql, parameters=(), /)¶
Create a new `Cursor` object and call
`execute()` on it with the given sql and parameters.
Return the new cursor object.

executemany(sql, parameters, /)¶
Create a new `Cursor` object and call
`executemany()` on it with the given sql and parameters.
Return the new cursor object.

executescript(sql_script, /)¶
Create a new `Cursor` object and call
`executescript()` on it with the given sql_script.
Return the new cursor object.
...
* deterministic (bool) – If `True`, the created SQL function is marked as
deterministic,
...
Changed in version 3.13: Passing name, n_arg, and aggregate_class as keyword arguments is deprecated.
These parameters will become positional-only in Python 3.15.
...
Raises:
NotSupportedError – If used with a version of SQLite older than 3.25.0,
...
Changed in version 3.13: Passing authorizer_callback as a keyword argument is deprecated.
...
Changed in version 3.13: Passing progress_handler as a keyword argument is deprecated.
The parameter will become positional-only in Python 3.15.

set_trace_callback(trace_callback)¶
Register callable trace_callback to be invoked
for each SQL statement that is actually executed by the SQLite backend.

The only argument passed to the callback is the statement (as
`str`) that is being executed. The return value of the callback is
ignored. Note that the backend does not only run statements passed to the
`Cursor.execute()` methods.  Other sources include the
...
`enable_callback_tracebacks()` to enable printing
tracebacks from exceptions raised in the trace callback.

Added in version 3.3.

Changed in version 3.13: Passing trace_callback as a keyword argument is deprecated.
The parameter will become positional-only in Python 3.15.
...
you must pass the `--enable-loadable-sqlite-extensions` option
to configure.
...
Enable extension loading with `enable_load_extension()` before
calling this method.
...
Parameters:
filter (str | None) – An optional `LIKE` pattern for database objects to dump, e.g. `prefix_%`.
...
* sleep (float) – The number of seconds to sleep between successive attempts
...
for `Connection` `con` (the default is 1000000000):
...
for `Connection` `con` (the default limit is 10):
...
* enable (bool) – `True` if the configuration option should be enabled (default);
`False` if it should be disabled.

Added in version 3.12.

serialize(*, name='main')¶
Serialize a database into a `bytes` object.  For an
...
Deserialize a `serialized` database into a
`Connection`.
...
* OperationalError – If the database connection is currently involved in a read
...
`commit()` and `rollback()` have no effect in this mode.
...
`isolation_level` has no effect unless `autocommit` is
set to `LEGACY_TRANSACTION_CONTROL` (the default).

row_factory¶
The initial `row_factory`
for `Cursor` objects created from this connection.
...
text_factory¶
A callable that accepts a `bytes` parameter
...
or by using any of the connection shortcut methods.

Cursor objects are iterators,
meaning that if you `execute()` a `SELECT` query,
...
When named placeholders are used
...
Changed in version 3.14: `ProgrammingError` is emitted if
...
repeatedly execute the parameterized
...
* parameters (iterable) – An iterable of parameters to bind with
...
including DML statements with RETURNING clauses.
...
If size is not given, `arraysize` determines the number of rows
to be fetched.
...
belonging to the cursor.  A `Cursor` object created by
...
using the `execute()` method.  For other statements, after
`executemany()` or `executescript()`, or if the insertion failed,
...
argument defaults to `os.SEEK_SET` (absolute blob positioning).
Other values for origin are `os.SEEK_CUR` (seek relative to the
current position) and `os.SEEK_END` (seek relative to the blob’s
end).
...
adaption protocol for objects that can adapt themselves to native SQLite types.
...
`ProgrammingError` is a subclass of `DatabaseError`.

exception sqlite3.NotSupportedError¶
Exception raised in case a method or database API is not supported by the
underlying SQLite library. For example, setting deterministic to
`True` in `create_function()`, if the underlying SQLite library
...
Instead, use the Adapter and converter recipes
