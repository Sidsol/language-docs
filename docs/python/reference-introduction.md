---
sourceUrl: https://docs.python.org/3/reference/introduction.html
...
## 1.1. Alternate Implementations[¶](#alternate-implementations)
...
**name**: [`letter`](#grammar-token-notation-letter) ([`letter`](#grammar-token-notation-letter) | [`digit`](#grammar-token-notation-digit) | "_")*
**letter**: "a"..."z" | "A"..."Z"
**digit**: "0"..."9"
...
* `TOKEN`: An uppercase name refers to a [token](../glossary.html#term-token).
...
* `'if'`: A name in single quotes denotes a [keyword](lexical_analysis.html#keywords).

* `"case"`: A name in double quotes denotes a
[soft-keyword](lexical_analysis.html#soft-keywords).

* `''`: A non-letter symbol in single quotes denotes an
[`OP`](../library/token.html#token.OP) token, that is, a [delimiter](lexical_analysis.html#delimiters) or
[operator](lexical_analysis.html#operators).
...
### 1.2.1. Lexical and Syntactic definitions[¶](#lexical-and-syntactic-definitions)

There is some difference between *lexical* and *syntactic* analysis:
the [lexical analyzer](../glossary.html#term-lexical-analyzer) operates on the individual characters of the
input source, while the *parser* (syntactic analyzer) operates on the stream
of [tokens](../glossary.html#term-token) generated by the lexical analysis.
...
The lexical analyzer [discards](lexical_analysis.html#whitespace) all whitespace that is not
converted to tokens like [`token.INDENT`](../library/token.html#token.INDENT) or [`NEWLINE`](../library/token.html#token.NEWLINE).
...
All uses of BNF in the next chapter ([Lexical analysis](lexical_analysis.html#lexical)) are lexical definitions;
...
*
[Show source
](https://github.com/python/cpython/blob/main/Doc/reference/introduction.rst?plain=1)
