...
Package DateTypeThird-party library that introduces distinct static types to for example,
allow static type checkers
...
Equivalent to `timedelta(-t1.days,
...
platforms, this method may raise `OverflowError` or `OSError`
...
`zoneinfo`The `datetime` module has a basic `timezone` class (for
handling arbitrary fixed offsets from UTC) and its `timezone.utc`
attribute (a UTC `timezone` instance).

`zoneinfo` brings the IANA time zone database (also known as the Olson
database) to Python, and its usage is recommended.

IANA time zone databaseThe Time Zone Database (often called tz, tzdata or zoneinfo) contains code
...
`%z``utcoffset()` is transformed into a string of the form
...
`%:z`Behaves exactly as `%z`, but has a colon separator added between
hours, minutes and seconds.

`%Z`In `strftime()`, `%Z` is replaced by an empty string if
...
Deprecated since version 3.13, will be removed in version 3.15: `strptime()` calls using a format string containing
