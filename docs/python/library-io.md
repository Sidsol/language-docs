---
sourceUrl: https://docs.python.org/3/library/io.html
...
Python UTF-8 ModePython UTF-8 Mode can be used to change the default encoding to
...
`sys`contains the standard IO streams: `sys.stdin`, `sys.stdout`,
and `sys.stderr`.
...
The `BufferedIOBase` ABC extends `IOBase`.  It deals with
buffering on a raw binary stream (`RawIOBase`).  Its subclasses,
`BufferedWriter`, `BufferedReader`, and `BufferedRWPair`
buffer raw binary streams that are writable, readable, and both readable and writable,
respectively. `BufferedRandom` provides a buffered interface to seekable streams.
Another `BufferedIOBase` subclass, `BytesIO`, is a stream of
in-memory bytes.
...
from strings.  `TextIOWrapper`, which extends `TextIOBase`, is a buffered text
...
* `os.SEEK_CUR` or `1` – current stream position;
offset may be negative

* `os.SEEK_END` or `2` – end of the stream;
...
Fewer bytes may be returned than requested. An empty `bytes` object
...
`os.read()` and PEP 475 for more details. Less than size bytes
...
either raise `BlockingIOError` or return `None` if no data is
available. `io` implementations return `None`.

read1(size=-1, /)¶
Read and return up to size bytes, calling `readinto()`
which may retry if `EINTR` is encountered per
...
`readinto()` and `write()` methods on this
...
`BlockingIOError` with `BlockingIOError.characters_written` set
...
`RawIOBase` raw binary stream.  It inherits from `BufferedReader`
and `BufferedWriter`.
...
`BufferedWriter` can do.  In addition, `seek()` and
`tell()` are guaranteed to be implemented.
...
or `RawIOBase` instance) that `TextIOBase` deals with.
...
explicitly. See Text Encoding for more information.
...
The return value of `tell()` can be given as input to `seek()`,
...
The text buffer is discarded when the `close()` method is
called.

The initial value of the buffer can be set by providing initial_value.
If newline translation is enabled, newlines will be encoded as if by
`write()`.  The stream is positioned at the start of the
...
See ABCs and Protocols for working with I/O for other I/O related protocols and classes that can be
...
`tell()` and `seek()` are both quite slow
...
standard streams and therefore affects the built-in `print()` function as
well.