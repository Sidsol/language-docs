...
way. Functions such as [`importlib.import_module()`](../library/importlib.html#importlib.import_module) and built-in
[`__import__()`](../library/functions.html#import__) can also be used to invoke the import machinery.
...
a call to the [`__import__()`](../library/functions.html#import__) function, with the appropriate arguments.
...
A direct call to [`__import__()`](../library/functions.html#import__) performs only the module search and, if
found, the module creation operation. While certain side-effects may occur,
such as the importing of parent packages, and the updating of various caches
(including [`sys.modules`](../library/sys.html#sys.modules)), only the [`import`](simple_stmts.html#import) statement performs
a name binding operation.

When an [`import`](simple_stmts.html#import) statement is executed, the standard builtin
[`__import__()`](../library/functions.html#import__) function is called. Other mechanisms for invoking the
import system (such as [`importlib.import_module()`](../library/importlib.html#importlib.import_module)) may choose to bypass
...
cannot be found, a [`ModuleNotFoundError`](../library/exceptions.html#ModuleNotFoundError) is raised. Python implements various
...
import system is exposed through [`sys.meta_path`](../library/sys.html#sys.meta_path). In addition,
native namespace package support has been implemented (see [**PEP 420**](https://peps.python.org/pep-0420/)).

## 5.1. [`importlib`](../library/importlib.html#module-importlib)[¶](#importlib)

The [`importlib`](../library/importlib.html#module-importlib) module provides a rich API for interacting with the
import system. For example [`importlib.import_module()`](../library/importlib.html#importlib.import_module) provides a
recommended, simpler API than built-in [`__import__()`](../library/functions.html#import__) for invoking the
...
you might have a package called [`email`](../library/email.html#module-email), which in turn has a subpackage
called [`email.mime`](../library/email.mime.html#module-email.mime) and a module within that subpackage called
[`email.mime.text`](../library/email.mime.html#module-email.mime.text).

### 5.2.1. Regular packages[¶](#regular-packages)
...
### 5.2.2. Namespace packages[¶](#namespace-packages)
...
that package if the path of their parent package (or [`sys.path`](../library/sys.html#sys.path) for a
...
[`importlib.import_module()`](../library/importlib.html#importlib.import_module) or [`__import__()`](../library/functions.html#import__) functions.

This name will be used in various phases of the import search, and it may be
the dotted path to a submodule, e.g. `foo.bar.baz`. In this case, Python
first tries to import `foo`, then `foo.bar`, and finally `foo.bar.baz`.
If any of the intermediate imports fail, a [`ModuleNotFoundError`](../library/exceptions.html#ModuleNotFoundError) is raised.

### 5.3.1. The module cache[¶](#the-module-cache)

The first place checked during import search is [`sys.modules`](../library/sys.html#sys.modules). This
...
During import, the module name is looked up in [`sys.modules`](../library/sys.html#sys.modules) and if
present, the associated value is the module satisfying the import, and the
process completes. However, if the value is `None`, then a
[`ModuleNotFoundError`](../library/exceptions.html#ModuleNotFoundError) is raised. If the module name is missing, Python will
continue searching for the module.

[`sys.modules`](../library/sys.html#sys.modules) is writable. Deleting a key may not destroy the
...
of the module to result in a [`ModuleNotFoundError`](../library/exceptions.html#ModuleNotFoundError).

Beware though, as if you keep a reference to the module object,
invalidate its cache entry in [`sys.modules`](../library/sys.html#sys.modules), and then re-import the
named module, the two module objects will *not* be the same. By contrast,
[`importlib.reload()`](../library/importlib.html#importlib.reload) will reuse the *same* module object, and simply
reinitialise the module contents by rerunning the module’s code.

### 5.3.2. Finders and loaders[¶](#finders-and-loaders)

If the named module is not found in [`sys.modules`](../library/sys.html#sys.modules), then Python’s import
...
### 5.3.3. Import hooks[¶](#import-hooks)
...
import processing has occurred, other than [`sys.modules`](../library/sys.html#sys.modules) cache look up.
This allows meta hooks to override [`sys.path`](../library/sys.html#sys.path) processing, frozen
modules, or even built-in modules. Meta hooks are registered by adding new
finder objects to [`sys.meta_path`](../library/sys.html#sys.meta_path), as described below.

Import path hooks are called as part of [`sys.path`](../library/sys.html#sys.path) (or
...
to [`sys.path_hooks`](../library/sys.html#sys.path_hooks) as described below.

### 5.3.4. The meta path[¶](#the-meta-path)

When the named module is not found in [`sys.modules`](../library/sys.html#sys.modules), Python next
searches [`sys.meta_path`](../library/sys.html#sys.meta_path), which contains a list of meta path finder
objects. These finders are queried in order to see if they know how to handle
the named module. Meta path finders must implement a method called
[`find_spec()`](../library/importlib.html#importlib.abc.MetaPathFinder.find_spec) which takes three arguments:
...
[`sys.meta_path`](../library/sys.html#sys.meta_path) processing reaches the end of its list without returning
a spec, then a [`ModuleNotFoundError`](../library/exceptions.html#ModuleNotFoundError) is raised. Any other exceptions
raised are simply propagated up, aborting the import process.

The [`find_spec()`](../library/importlib.html#importlib.abc.MetaPathFinder.find_spec) method of meta path
...
be accessed, a [`ModuleNotFoundError`](../library/exceptions.html#ModuleNotFoundError) is raised. The third argument
...
Python’s default [`sys.meta_path`](../library/sys.html#sys.meta_path) has three meta path finders, one that
...
Changed in version 3.4: The [`find_spec()`](../library/importlib.html#importlib.abc.MetaPathFinder.find_spec) method of meta path
...
now raises [`ImportWarning`](../library/exceptions.html#ImportWarning).

Changed in version 3.12: `find_module()` has been removed.
Use [`find_spec()`](../library/importlib.html#importlib.abc.MetaPathFinder.find_spec) instead.
...
[`sys.modules`](../library/sys.html#sys.modules), import will have already returned it.

* The module will exist in [`sys.modules`](../library/sys.html#sys.modules) before the loader
...
gets removed from [`sys.modules`](../library/sys.html#sys.modules). Any module already in the
...
[`importlib.abc.Loader.load_module()`](../library/importlib.html#importlib.abc.Loader.load_module) method.

### 5.4.1. Loaders[¶](#loaders)

Module loaders provide the critical function of loading: module execution.
The import machinery calls the [`importlib.abc.Loader.exec_module()`](../library/importlib.html#importlib.abc.Loader.exec_module)
...
[`ImportError`](../library/exceptions.html#ImportError), although any other exception raised during
[`exec_module()`](../library/importlib.html#importlib.abc.Loader.exec_module) will be propagated.

In many cases, the finder and loader can be the same object; in such cases the
[`find_spec()`](../library/importlib.html#importlib.abc.MetaPathFinder.find_spec) method would just return a
spec with the loader set to `self`.

Module loaders may opt in to creating the module object during loading
by implementing a [`create_module()`](../library/importlib.html#importlib.abc.Loader.create_module) method.
...
Added in version 3.4: The [`create_module()`](../library/importlib.html#importlib.abc.Loader.create_module) method of loaders.

Changed in version 3.4: The [`load_module()`](../library/importlib.html#importlib.abc.Loader.load_module) method was replaced by
[`exec_module()`](../library/importlib.html#importlib.abc.Loader.exec_module) and the import
...
* If there is an existing module object with the given name in
[`sys.modules`](../library/sys.html#sys.modules), the loader must use that existing module.
(Otherwise, [`importlib.reload()`](../library/importlib.html#importlib.reload) will not work correctly.) If the
named module does not exist in `sys.modules`, the loader
must create a new module object and add it to `sys.modules`.

* The module *must* exist in [`sys.modules`](../library/sys.html#sys.modules) before the loader
executes the module code, to prevent unbounded recursion or multiple
loading.

* If loading fails, the loader must remove any modules it has inserted
into [`sys.modules`](../library/sys.html#sys.modules), but it must remove **only** the failing
module(s), and only if the loader itself has loaded the module(s)
explicitly.

Changed in version 3.5: A [`DeprecationWarning`](../library/exceptions.html#DeprecationWarning) is raised when `exec_module()` is defined but
`create_module()` is not.

Changed in version 3.6: An [`ImportError`](../library/exceptions.html#ImportError) is raised when `exec_module()` is defined but
`create_module()` is not.

Changed in version 3.10: Use of `load_module()` will raise [`ImportWarning`](../library/exceptions.html#ImportWarning).

### 5.4.2. Submodules[¶](#submodules)
...
### 5.4.3. Module specs[¶](#module-specs)
...
The module’s spec is exposed as [`module.__spec__`](datamodel.html#module.__spec__). Setting
...
See [`ModuleSpec`](../library/importlib.html#importlib.machinery.ModuleSpec) for details on the contents of
the module spec.

Added in version 3.4.

### 5.4.4. __path__ attributes on modules[¶](#path-attributes-on-modules)

The [`__path__`](datamodel.html#module.__path__) attribute should be a (possibly empty)
...
A package’s [`__path__`](datamodel.html#module.__path__) attribute is used during imports of its
subpackages.
Within the import machinery, it functions much the same as [`sys.path`](../library/sys.html#sys.path),
i.e. providing a list of locations to search for modules during import.
However, `__path__` is typically much more constrained than
`sys.path`.

The same rules used for [`sys.path`](../library/sys.html#sys.path) also apply to a package’s
`__path__`. [`sys.path_hooks`](../library/sys.html#sys.path_hooks) (described below) are
consulted when traversing a package’s `__path__`.

A package’s `__init__.py` file may set or alter the package’s
[`__path__`](datamodel.html#module.__path__)
...
### 5.4.5. Module reprs[¶](#module-reprs)
...
### 5.4.6. Cached bytecode invalidation[¶](#cached-bytecode-invalidation)
...
## 5.5. The Path Based Finder[¶](#the-path-based-finder)
...
([`PathFinder`](../library/importlib.html#importlib.machinery.PathFinder)), searches an [import path](../glossary.html#term-import-path),
...
shared libraries (e.g. `.so` files). When supported by the [`zipimport`](../library/zipimport.html#module-zipimport)
...
process, as keyed off the [`sys.meta_path`](../library/sys.html#sys.meta_path) traversal.
...
removed from [`sys.meta_path`](../library/sys.html#sys.meta_path), none of the path entry finder semantics
would be invoked.

### 5.5.1. Path entry finders[¶](#path-entry-finders)
...
As a meta path finder, the [path based finder](../glossary.html#term-path-based-finder) implements the
[`find_spec()`](../library/importlib.html#importlib.abc.MetaPathFinder.find_spec) protocol previously
...
Three variables are used by the [path based finder](../glossary.html#term-path-based-finder), [`sys.path`](../library/sys.html#sys.path),
[`sys.path_hooks`](../library/sys.html#sys.path_hooks) and [`sys.path_importer_cache`](../library/sys.html#sys.path_importer_cache). The `__path__`
attributes on package objects are also used. These provide additional ways
that the import machinery can be customized.

[`sys.path`](../library/sys.html#sys.path) contains a list of strings providing search locations for
...
(see the [`site`](../library/site.html#module-site) module) that should be searched for modules, such as
...
based finder’s [`find_spec()`](../library/importlib.html#importlib.machinery.PathFinder.find_spec) method as
...
`None`, this indicates a top level import and [`sys.path`](../library/sys.html#sys.path) is used.
...
([`PathEntryFinder`](../library/importlib.html#importlib.abc.PathEntryFinder)) for the
path entry. Because this can be an expensive operation (e.g. there may be
`stat()` call overheads for this search), the path based finder maintains
a cache mapping path entries to path entry finders. This cache is maintained
in [`sys.path_importer_cache`](../library/sys.html#sys.path_importer_cache) (despite the name, this cache actually
...
over every callable in [`sys.path_hooks`](../library/sys.html#sys.path_hooks). Each of the [path entry
...
[`ImportError`](../library/exceptions.html#ImportError). An `ImportError` is used by the path based finder to
...
If [`sys.path_hooks`](../library/sys.html#sys.path_hooks) iteration ends with no [path entry finder](../glossary.html#term-path-entry-finder)
being returned, then the path based finder’s
[`find_spec()`](../library/importlib.html#importlib.machinery.PathFinder.find_spec) method will store `None`
in [`sys.path_importer_cache`](../library/sys.html#sys.path_importer_cache) (to indicate that there is no finder for
...
hook](../glossary.html#term-path-entry-hook) callables on [`sys.path_hooks`](../library/sys.html#sys.path_hooks), then the following protocol is used
to ask the finder for a module spec, which is then used when loading the
module.

The current working directory – denoted by an empty string – is handled
slightly differently from other entries on [`sys.path`](../library/sys.html#sys.path). First, if the
current working directory cannot be determined or is found not to exist, no
value is stored in [`sys.path_importer_cache`](../library/sys.html#sys.path_importer_cache). Second, the value for the
...
[`importlib.machinery.PathFinder.find_spec()`](../library/importlib.html#importlib.machinery.PathFinder.find_spec) will be the actual current
working directory and not the empty string.

### 5.5.2. Path entry finder protocol[¶](#path-entry-finder-protocol)

In order to support imports of modules and initialized packages and also to
contribute portions to namespace packages, path entry finders must implement
the [`find_spec()`](../library/importlib.html#importlib.abc.PathEntryFinder.find_spec) method.

[`find_spec()`](../library/importlib.html#importlib.abc.PathEntryFinder.find_spec) takes two arguments: the
...
Changed in version 3.4: [`find_spec()`](../library/importlib.html#importlib.abc.PathEntryFinder.find_spec) replaced
`find_loader()` and
...
system will raise [`ImportWarning`](../library/exceptions.html#ImportWarning).

Changed in version 3.12: `find_module()` and `find_loader()` have been removed.

## 5.6. Replacing the standard import system[¶](#replacing-the-standard-import-system)

The most reliable mechanism for replacing the entire import system is to
delete the default contents of [`sys.meta_path`](../library/sys.html#sys.meta_path), replacing them
entirely with a custom meta path hook.

If it is acceptable to only alter the behaviour of import statements
without affecting other APIs that access the import system, then replacing
the builtin [`__import__()`](../library/functions.html#import__) function may be sufficient.

To selectively prevent the import of some modules from a hook early on the
meta path (rather than disabling the standard import system entirely),
it is sufficient to raise [`ModuleNotFoundError`](../library/exceptions.html#ModuleNotFoundError) directly from
[`find_spec()`](../library/importlib.html#importlib.abc.MetaPathFinder.find_spec) instead of returning
`None`. The latter indicates that the meta path search should continue,
while raising an exception terminates it immediately.

## 5.7. Package Relative Imports[¶](#package-relative-imports)
...
## 5.8. Special considerations for __main__[¶](#special-considerations-for-main)

The [`__main__`](../library/__main__.html#module-__main__) module is a special case relative to Python’s import
system. As noted [elsewhere](toplevel_components.html#programs), the `__main__` module
is directly initialized at interpreter startup, much like [`sys`](../library/sys.html#module-sys) and
[`builtins`](../library/builtins.html#module-builtins). However, unlike those two, it doesn’t strictly
...
### 5.8.1. __main__.__spec__[¶](#main-spec)

Depending on how [`__main__`](../library/__main__.html#module-__main__) is initialized, `__main__.__spec__`
...
directory, zipfile or other [`sys.path`](../library/sys.html#sys.path) entry.
...
[`__main__`](../library/__main__.html#module-__main__) does not correspond directly with an importable module:

* interactive prompt

* [`-c`](../using/cmdline.html#cmdoption-c) option
...
in [`__main__`](../library/__main__.html#module-__main__).
...
The original specification for [`sys.meta_path`](../library/sys.html#sys.meta_path) was [**PEP 302**](https://peps.python.org/pep-0302/), with
subsequent extension in [**PEP 420**](https://peps.python.org/pep-0420/).

[**PEP 420**](https://peps.python.org/pep-0420/) introduced [namespace packages](../glossary.html#term-namespace-package) for
...
[[1](#id1)]
See [`types.ModuleType`](../library/types.html#types.ModuleType).
...
in [`sys.modules`](../library/sys.html#sys.modules). The indirect effect of this is that an imported
...
*
[Show source
](https://github.com/python/cpython/blob/main/Doc/reference/import.rst?plain=1)
