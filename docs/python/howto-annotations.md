---
sourceUrl: https://docs.python.org/3/howto/annotations.html
...
If for some reason `inspect.get_annotations()` isn't viable for your use case, you may access the `__annotations__` data member manually. Best practice for this changed in Python 3.10 as well: as of Python 3.10, `o.__annotations__` is guaranteed to _always_ work on Python functions, classes, and modules. If you're certain the object you're examining is one of these three _specific_ objects, you may simply use `o.__annotations__` to get at the object's annotations dict.
...
However, not all string values used as annotations can be successfully turned into Python values by `eval()`. String values could theoretically contain any valid string, and in practice there are valid use cases for type hints that require annotating with string values that specifically _can't_ be evaluated.
