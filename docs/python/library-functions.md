---
sourceUrl: https://docs.python.org/3/library/functions.html
...
(`__module__`, `__name__`,
`__qualname__`, `__doc__` and
`__annotations__`) and have a new `__wrapped__`
attribute.

Deprecated since version 3.11, removed in version 3.13: Class methods can no longer wrap other descriptors such as
`property()`.
...
`ValueError`.
More precisely, the input must conform to the `complexvalue`
...
More precisely, the input must conform to the `floatvalue`
...
If the argument defines `__int__()`,
`int(x)` returns `x.__int__()`.  If the argument defines
`__index__()`, it returns `x.__index__()`.
...
optimized scopes is now as described above.
...
`object` instances do not have `__dict__`
...
dependent (whatever `locale.getencoding()` returns), but any
text encoding supported by Python can be used.
...
`__len__()` method and the `__getitem__()` method
...
name need not be a Python identifier as defined in Names (identifiers and keywords)
...
Changed in version 3.12: Slice objects are now hashable (provided `start`,
`stop`, and `step` are hashable).
...
(`__module__`, `__name__`,
`__qualname__`, `__doc__` and
`__annotations__`), have a new `__wrapped__` attribute,
...
For example, if `__mro__` of object_or_type is
