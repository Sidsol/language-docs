---
sourceUrl: https://docs.python.org/3/library/math.html
...
for negative x. If x is not a float, delegates to `x.__trunc__`, which should return an `Integral` value.
...
`float('nan')`. Due to the requirements of the IEEE-754 standard, `math.nan` and `float('nan')` are
