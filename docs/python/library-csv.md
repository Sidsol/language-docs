...
the Dialects and Formatting Parameters section. To make it
...
columns + rows meet the criteria, `True` is returned.
...
Some numeric types, such as `bool`, `Fraction`,
or `IntEnum`, have a string representation that cannot be
converted to `float`.
...
subclass of the `Dialect` class containing various attributes
describing the format of the CSV file.  When creating `reader` or
...
reader.  It can take on any of the QUOTE_* constants
and defaults to `QUOTE_MINIMAL` if quotechar is not `None`,
and `QUOTE_NONE` otherwise.
...
instance), parsed according to the current `Dialect`.  Usually you
...
to the current `Dialect`. Return the return value of the call to the
...
encoding (see `locale.getencoding()`).  To decode a file
