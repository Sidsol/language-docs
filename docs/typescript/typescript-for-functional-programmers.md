---
sourceUrl: "https://www.typescriptlang.org/docs/handbook/typescript-in-5-minutes-func.html"
fetchedAt: "2026-04-19T23:34:50.756Z"
language: "TypeScript"
...
`T[]`
mutable arrays, also written `Array`
...
```
tslet fst: (a: any, b: any) => any = (a, b) => a;// or more precisely:let fst: (a: T, b: U) => T = (a, b) => a;
```
...
```
tstype One = { p: string };interface Two {  p: string;}class Three {  p = "Hello";} let x: One = { p: "hi" };let two: Two = x;two = new Three();Try
```
...
```
tstype Combined = { a: number } & { b: string };type Conflicting = { a: number } & { a: string };Try
```
...
```
tsdeclare function pad(s: string, n: number, direction: "left" | "right"): string;pad("hi", 10, "left");Try
```
...
```
tslet s = "right";pad("hi", 10, s); // error: &apos;string&apos; is not assignable to &apos;"left" | "right"&apos;Argument of type 'string' is not assignable to parameter of type '"left" | "right"'.2345Argument of type 'string' is not assignable to parameter of type '"left" | "right"'.Try
```
...
```
tslet s: "left" | "right" = "right";pad("hi", 10, s);Try
```
...
```
tsdeclare function (f: (t: T) => U, ts: T[]): U[]' >map U, ts: T[]): U[]' >T, (f: (t: T) => U, ts: T[]): U[]' >U>( U' >f: (t: (f: (t: T) => U, ts: T[]): U[]' >T) => (f: (t: T) => U, ts: T[]): U[]' >U, ts: (f: (t: T) => U, ts: T[]): U[]' >T[]): (f: (t: T) => U, ts: T[]): U[]' >U[];let sns = (f: (t: number) => string, ts: number[]): string[]' >map((n) => n.toString(), [1, 2, 3]);Try
```
...
```
tsdeclare function (ts: T[], f: (t: T) => U): U[]' >map U): U[]' >T, (ts: T[], f: (t: T) => U): U[]' >U>(ts: (ts: T[], f: (t: T) => U): U[]' >T[],  U' >f: (t: (ts: T[], f: (t: T) => U): U[]' >T) => (ts: T[], f: (t: T) => U): U[]' >U): (ts: T[], f: (t: T) => U): U[]' >U[];Try
```
...
```
tsdeclare function (thunk: (t: T) => void): T' >run void): T' >T>( void' >thunk: (t: (thunk: (t: T) => void): T' >T) => void): (thunk: (t: T) => void): T' >T;let i: { inference: string } = (thunk: (t: {
    inference: string;
}) => void): {
    inference: string;
...
```
tstype Size = [number, number];let x: Size = [101.1, 999.9];Try
```
...
```
tstype Shape =  | { kind: "circle"; radius: number }  | { kind: "square"; x: number }  | { kind: "triangle"; x: number; y: number }; function area(s: Shape) {  if (s.kind === "circle") {    return Math.PI * s.radius * s.radius;  } else if (s.kind === "square") {    return s.x * s.x;  } else {    return (s.x * s.y) / 2;  }}Try
```
...
```
tsfunction height(s: Shape) {  if (s.kind === "circle") {    return 2 * s.radius;  } else {    // s.kind: "square" | "triangle"    return s.x;  }}Try
```
...
```
tsfunction liftArray(t: T): Array {  return [t];}
```
...
```
tsfunction firstish(t1: T, t2: T): T {  return t1.length > t2.length ? t1 : t2;}
```
...
```
tsfunction length>(t: T): number {}function length(t: ArrayLike): number {}
```
...
```
tsinterface X {  x: number;}let rx: Readonly = { x: 1 };rx.x = 12; // error
```
...
```
tslet a: ReadonlyArray = [1, 2, 3];let b: readonly number[] = [1, 2, 3];a.push(102); // errorb[0] = 101; // error
```
