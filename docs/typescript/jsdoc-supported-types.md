...
fetchedAt: "2026-04-19T23:34:54.505Z"
language: "TypeScript"
...
```
js/** * @type {string} */var s; /** @type {Window} */var win; /** @type {PromiseLike} */var ' >promisedString; // You can specify an HTML Element with DOM properties/** @type {HTMLElement} */var myElement = document.(selectors: any): any (+4 overloads)' >querySelector(selector);element.dataset.myData = "";Try
```
...
```
js/** * @type {string | boolean} */var sb;Try
```
...
```
js/** @type {number[]} */var ns;/** @type {Array.} */var jsdoc;/** @type {Array} */var nas;Try
```
...
```
js/** @type {{ a: string, b: number }} */var var9;Try
```
...
```
js/** * A map-like object that maps arbitrary `string` properties to `number`s. * * @type {Object.} */var stringToNumber; /** @type {Object.} */var arrayLike;Try
```
...
```
js/** @type {function(string, boolean): number} Closure syntax */var  number' >sbn;/** @type {(s: string, b: boolean) => number} TypeScript syntax */var  number' >sbn2;Try
```
...
```
js/** @type {Function} */var fn7;/** @type {function} */var fn6;Try
```
...
```
js/** * @type {*} - can be &apos;any&apos; type */var star;/** * @type {?} - unknown type (same as &apos;any&apos;) */var question;Try
```
...
```
js/** * @type {number | string} */var numberOrString = Math.random() require("./accounts").userAccount;Try
```
...
```
js/** * @import {Pet} from "./types" */ /** * @type {Pet} */var myPet;myPet.name;Try
```
...
```
js// @filename: dog.jsexport class Dog {  woof() {    console.log("Woof!");  }} // @filename: main.js/** @import { Dog } from "./dog.js" */ const d = new Dog(); // error!Try
```
...
```
js/** * @return {PromiseLike} */function ' >ps() {} /** * @returns {{ a: string, b: number }} - May use &apos;@returns&apos; as well as &apos;@return&apos; */function ab() {}Try
```
...
```
js/** * @typedef {object} SpecialType1 - creates a new type named &apos;SpecialType1&apos; * @property {string} prop1 - a string property of SpecialType1 * @property {number} prop2 - a number property of SpecialType1 * @property {number=} prop3 - an optional number property of SpecialType1 */ /** @type {SpecialType1} */var specialTypeObject1;Try
```
...
```
js/** * @param {Object} options - The shape is the same as SpecialType above * @param {string} options.prop1 * @param {number} options.prop2 * @param {number=} options.prop3 * @param {number} [options.prop4] * @param {number} [options.prop5=42] */function special(options) {  return (options.prop4 || 1001) + options.prop5;}Try
```
...
```
js/** * @callback Predicate * @param {string} data * @param {number} [index] * @returns {boolean} */ /** @type {Predicate} */const ok = (s) => !(s.length % 2);Try
```
...
```
js/** * @template T * @param {T} x - A generic parameter that flows through to the return type * @returns {T} */function (x: T): T' >id(x) {  return x;} const a = (x: "string"): "string"' >id("string");const b = (x: 123): 123' >id(123);const c = (x: {}): {}' >id({});Try
```
...
```
js/** * @template {string} K - K must be a string or string literal * @template {{ serious(): string }} Seriousalizable - must have a serious method * @param {K} key * @param {Seriousalizable} object */function (key: K, object: Seriousalizable): void' >seriousalize(key, object) {  // ????}Try
```
...
```
js/** @template [T=object] */class ' >Cache {    /** @param {T} initial */    constructor(initial) {    }}let c = new  Cache' >Cache()Try
```
...
```
jsclass C {  /**   * @param {number} data   */  constructor(data) {    // property types can be inferred    this.name = "foo";     // or set explicitly    /** @type {string | null} */    this.title = null;     // or simply annotated, if they&apos;re set elsewhere    /** @type {number} */    this.size;     this. void' >initialize(data); // Should error, initializer expects a string  }  /**   * @param {string} s   */  initialize = function (s) {    this.size = s.length;  };} var c = new C(0); // C should only be called with new, but// because it is JavaScript, this is allowed and// considered an &apos;any&apos;.var result = C(1);Try
```
...
```
js// @ts-check class Car {  constructor() {    /** @private */    this.identifier = 100;  }   printIdentifier() {    console.log(this.identifier);  }} const c = new Car();console.log(c.identifier);Property 'identifier' is private and only accessible within class 'Car'.2341Property 'identifier' is private and only accessible within class 'Car'.Try
```
...
```
js// @ts-check class Car {  constructor() {    /** @readonly */    this.identifier = 100;  }   printIdentifier() {    console.log(this.identifier);  }} const c = new Car();console.log(c.identifier);Try
```
...
```
jsexport class C {  m() { }}class D extends C {  /** @override */  m() { }}Try
```
...
```
js/** * @template T * @extends {Set} */class ' >SortableSet extends Set {  // ...}Try
```
...
```
js/** @implements {Print} */class TextBook {  print() {    // TODO  }}Try
```
...
```
js/** * @constructor * @param {number} data */function C(data) {  // property types can be inferred  this.name = "foo";   // or set explicitly  /** @type {string | null} */  this.title = null;   // or simply annotated, if they&apos;re set elsewhere  /** @type {number} */  this.size;   this.initialize(data);Argument of type 'number' is not assignable to parameter of type 'string'.2345Argument of type 'number' is not assignable to parameter of type 'string'.}/** * @param {string} s */C.prototype.initialize = function (s) {  this.size = s.length;}; var c = new C(0);c.size; var result = C(1);Value of type 'typeof C' is not callable. Did you mean to include 'new'?2348Value of type 'typeof C' is not callable. Did you mean to include 'new'?Try
```
...
```
js/** * @this {HTMLElement} * @param {*} e */function callbackForLater(e) {  this.clientHeight = parseInt(e); // should be fine!}Try
```
...
```
js/** @deprecated */const apiV1 = {};const apiV2 = {}; apiV;   apiV1apiV2  Try
```
...
tstype  = {
    t: T;
}' >BoxT> = { t: ' >T }/** @see Box for implementation details */type  = { [K in keyof T]: Box; }' >BoxifyT> = { [K in keyof ' >T]:  = {
    t: T;
}' >BoxT> };Try
```
...
tstype  = {
    t: T;
}' >BoxT> = { t: ' >T }/** @returns A {@link Box} containing the parameter. */function (u: U): Box' >box' >U>(u: (u: U): Box' >U):  = {
    t: T;
}' >Box' >U> {  return { t: u };}Try
```
...
```
tstype  string;
}' >Pet = {  name: string   string' >hello: () => string} /** * Note: you should implement the {@link Pet.hello} method of Pet. */function hello(p:  string;
}' >Pet) {  p. string' >hello()}Try
```
...
```
tstype  string;
}' >Pet = {  name: string   string' >hello: () => string} /** * Note: you should implement the {@link Pet.hello | hello} method of Pet. */function hello(p:  string;
}' >Pet) {  p. string' >hello()}Try
```
...
```
js/** @enum {number} */const JSDocState = {  BeginningOfLine: 0,  SawAsterisk: 1,  SavingComments: 2,}; JSDocState.SawAsterisk;Try
```
...
```
js/** @enum {function(number): number} */const  number
const MathFuncs: {
    add1: (n: any) => any;
    id: (n: any) => number;
    sub1: (n: any) => number;
}' >MathFuncs = {   any' >add1: (n) => n + 1,   number' >id: (n) => -n,   number' >sub1: (n) => n - 1,};  any;
    id: (n: any) => number;
    sub1: (n: any) => number;
}' >MathFuncs. any' >add1;Try
```
...
```
ts/** * Welcome to awesome.ts * @author Ian Awesome  */Try
```
...
```
jsvar  void;
}' >someObj = {  /**   * @param {string} param1 - JSDocs on property assignments work   */   void' >x: function (param1) {},}; /** * As do jsdocs on variable assignments * @return {Window} */let  Window' >someFunc = function () {}; /** * And class methods * @param {string} greeting The greeting to use */Foo.prototype.sayHi = (greeting) => console.log("Hi!"); /** * And arrow function expressions * @param {number} x - A multiplier */let  number' >myArrow = (x) => x * x; /** * Which means it works for function components in JSX too * @param {{a: string, b: number}} props - Some param */var  React.JSX.Element' >fc = (props) => ' >div>{props.a.charAt(0)}' >div>; /** * A parameter can be a class constructor, using Google Closure syntax. * * @param {{new(...args: any[]): object}} C - The class to register */function registerClass( object' >C) {} /** * @param {...string} p1 - A &apos;rest&apos; arg (array) of strings. (treated as &apos;any&apos;) */function fn10(p1) {} /** * @param {...string} p1 - A &apos;rest&apos; arg (array) of strings. (treated as &apos;any&apos;) */function fn9(p1) {  return p1.join();}Try
```
...
```
js/** * @type {{ a: string, b: number= }} */var wrong;/** * Use postfix question on the property name instead: * @type {{ a: string, b?: number }} */var right;Try
```
...
```
js/** * @type {?number} * With strictNullChecks: true  -- number | null * With strictNullChecks: false -- number */var nullable;Try
```
...
```
js/** * @type {number | null} * With strictNullChecks: true  -- number | null * With strictNullChecks: false -- number */var unionNullable;Try
```
...
```
js/** * @type {!number} * Just has type number */var normal;Try
```
