...
fetchedAt: "2026-04-19T23:34:51.450Z"
language: "TypeScript"
title: "Generics"
---
# Generics
...
```
tsfunction (arg: Type): Type' >identityType>(arg: (arg: Type): Type' >Type): (arg: Type): Type' >Type {  return arg;}Try
```
...
```
tslet output = (arg: string): string' >identity("myString");      let output: stringTry
```
...
```
tslet output = (arg: "myString"): "myString"' >identity("myString");      let output: stringTry
```
...
```
tsfunction (arg: Type): Type' >loggingIdentityType>(arg: (arg: Type): Type' >Type): (arg: Type): Type' >Type {  console.log(arg.length);Property 'length' does not exist on type 'Type'.2339Property 'length' does not exist on type 'Type'.  return arg;}Try
```
...
```
tsfunction (arg: Type[]): Type[]' >loggingIdentityType>(arg: (arg: Type[]): Type[]' >Type[]): (arg: Type[]): Type[]' >Type[] {  console.log(arg..length: number' >length);  return arg;}Try
```
...
```
tsfunction (arg: Array): Array' >loggingIdentity): Array' >Type>(arg: ' >Array): Array' >Type>): ' >Array): Array' >Type> {  console.log(arg..length: number' >length); // Array has a .length, so no more error  return arg;}Try
```
...
```
tsfunction (arg: Type): Type' >identityType>(arg: (arg: Type): Type' >Type): (arg: Type): Type' >Type {  return arg;} let (arg: Type) => Type' >myIdentity: Type>(arg: (arg: Type): Type' >Type) => (arg: Type): Type' >Type = (arg: Type): Type' >identity;Try
```
...
```
tsfunction (arg: Type): Type' >identityType>(arg: (arg: Type): Type' >Type): (arg: Type): Type' >Type {  return arg;} let (arg: Input) => Input' >myIdentity: Input>(arg: (arg: Input): Input' >Input) => (arg: Input): Input' >Input = (arg: Type): Type' >identity;Try
```
...
```
tsinterface GenericIdentityFn {  Type>(arg: (arg: Type): Type' >Type): (arg: Type): Type' >Type;} function (arg: Type): Type' >identityType>(arg: (arg: Type): Type' >Type): (arg: Type): Type' >Type {  return arg;} let myIdentity: GenericIdentityFn = (arg: Type): Type' >identity;Try
```
...
```
tsinterface ' >GenericIdentityFnType> {  (arg: ' >Type): ' >Type;} function (arg: Type): Type' >identityType>(arg: (arg: Type): Type' >Type): (arg: Type): Type' >Type {  return arg;} let ' >myIdentity: ' >GenericIdentityFn = (arg: Type): Type' >identity;Try
```
...
```
tsclass ' >GenericNumberNumType> {  .zeroValue: NumType' >zeroValue: ' >NumType;  .add: (x: NumType, y: NumType) => NumType' >add: (x: ' >NumType, y: ' >NumType) => ' >NumType;} let ' >myGenericNumber = new (): GenericNumber' >GenericNumber();' >myGenericNumber..zeroValue: number' >zeroValue = 0;' >myGenericNumber..add: (x: number, y: number) => number' >add = function (x, y) {  return x + y;};Try
```
...
```
tslet ' >stringNumeric = new (): GenericNumber' >GenericNumber();' >stringNumeric..zeroValue: string' >zeroValue = "";' >stringNumeric..add: (x: string, y: string) => string' >add = function (x, y) {  return x + y;}; console.log(' >stringNumeric..add: (x: string, y: string) => string' >add(' >stringNumeric..zeroValue: string' >zeroValue, "test"));Try
```
...
```
tsinterface Lengthwise {  length: number;} function (arg: Type): Type' >loggingIdentityType extends Lengthwise>(arg: (arg: Type): Type' >Type): (arg: Type): Type' >Type {  console.log(arg.length); // Now we know it has a .length property, so no more error  return arg;}Try
```
...
```
ts(arg: Lengthwise): Lengthwise' >loggingIdentity(3);Argument of type 'number' is not assignable to parameter of type 'Lengthwise'.2345Argument of type 'number' is not assignable to parameter of type 'Lengthwise'.Try
```
...
```
tsfunction (obj: Type, key: Key): Type[Key]' >getPropertyType, (obj: Type, key: Key): Type[Key]' >Key extends keyof (obj: Type, key: Key): Type[Key]' >Type>(obj: (obj: Type, key: Key): Type[Key]' >Type, key: (obj: Type, key: Key): Type[Key]' >Key) {  return obj[key];} let x = { a: 1, b: 2, c: 3, d: 4 }; (obj: {
    a: number;
    b: number;
    c: number;
    d: number;
}, key: "a"): number' >getProperty(x, "a");(obj: {
    a: number;
    b: number;
    c: number;
    d: number;
}, key: "a" | "b" | "c" | "d"): number' >getProperty(x, "m");Argument of type '"m"' is not assignable to parameter of type '"a" | "b" | "c" | "d"'.2345Argument of type '"m"' is not assignable to parameter of type '"a" | "b" | "c" | "d"'.Try
```
...
tsfunction (c: {
    new (): Type;
}): Type' >createType>( Type' >c: { new (): (c: {
    new (): Type;
}): Type' >Type }): (c: {
    new (): Type;
}): Type' >Type {  return new  Type' >c();}Try
```
...
```
tsclass BeeKeeper {  hasMask: boolean = true;} class ZooKeeper {  nametag: string = "Mikle";} class Animal {  numLegs: number = 4;} class Bee extends Animal {  numLegs = 6;  keeper: BeeKeeper = new BeeKeeper();} class Lion extends Animal {  keeper: ZooKeeper = new ZooKeeper();} function (c: new () => A): A' >createInstance A): A' >A extends Animal>( A' >c: new () => (c: new () => A): A' >A): (c: new () => A): A' >A {  return new  A' >c();} (c: new () => Lion): Lion' >createInstance(Lion).keeper.nametag;(c: new () => Bee): Bee' >createInstance(Bee).keeper.hasMask;Try
```
...
```
tsdeclare function  (+2 overloads)' >create():  = {
    element: T;
    children: U;
}' >Container;declare function (element: T): Container (+2 overloads)' >create' >T extends HTMLElement>(element: (element: T): Container' >T):  = {
    element: T;
    children: U;
}' >Container' >T, (element: T): Container' >T[]>;declare function (element: T, children: U[]): Container (+2 overloads)' >create' >T extends HTMLElement, (element: T, children: U[]): Container' >U extends HTMLElement>(  element: (element: T, children: U[]): Container' >T,  children: (element: T, children: U[]): Container' >U[]):  = {
    element: T;
    children: U;
}' >Container' >T, (element: T, children: U[]): Container' >U[]>;Try
```
...
```
tsdeclare function (element?: T, children?: U): Container' >create' >T extends HTMLElement = HTMLDivElement, (element?: T, children?: U): Container' >U extends HTMLElement[] = (element?: T, children?: U): Container' >T[]>(  element?: (element?: T, children?: U): Container' >T,  children?: (element?: T, children?: U): Container' >U):  = {
    element: T;
    children: U;
}' >Container' >T, (element?: T, children?: U): Container' >U>; const ' style='border-bottom: solid 2px lightgrey;'>div = (element?: HTMLDivElement | undefined, children?: HTMLDivElement[] | undefined): Container' >create();      const div: Container const ' style='border-bottom: solid 2px lightgrey;'>p = (element?: HTMLParagraphElement | undefined, children?: HTMLParagraphElement[] | undefined): Container' >create(new  HTMLParagraphElement' >HTMLParagraphElement());     const p: ContainerTry
```
...
```
tsinterface Producer {  make(): T;}
```


We can use a `Producer` where a `Producer` is expected, because a `Cat` is an `Animal`.
This relationship is called *covariance*: the relationship from `Producer` to `Producer` is the same as the relationship from `T` to `U`.
...
```
tsinterface Consumer {  consume: (arg: T) => void;}
```
...
This relationship is called *contravariance*: the relationship from `Consumer` to `Consumer` is the same as the relationship from `U` to `T`.
...
```
tsinterface AnimalProducer {  make(): Animal;}// A CatProducer can be used anywhere an// Animal producer is expectedinterface CatProducer {  make(): Cat;}
```


TypeScript has a structural type system, so when comparing two types, e.g. to see if a `Producer` can be used where a `Producer` is expected, the usual algorithm would be structurally expand both of those definitions, and compare their structures.
However, variance allows for an extremely useful optimization: if `Producer` is covariant on `T`, then we can simply check `Cat` and `Animal` instead, as we know they’ll have the same relationship as `Producer` and `Producer`.


Note that this logic can only be used when we’re examining two instantiations of the same type.
If we have a `Producer` and a `FastProducer`, there’s no guarantee that `T` and `U` necessarily refer to the same positions in these types, so this check will always be performed structurally.
...
```
ts// Contravariant annotationinterface Consumer {  consume: (arg: T) => void;}// Covariant annotationinterface Producer {  make(): T;}// Invariant annotationinterface ProducerConsumer {  consume: (arg: T) => void;  make(): T;}
```
...
```
ts// DON'T DO THIS - variance annotation// does not match structural behaviorinterface Producer {  make(): T;}// Not a type error -- this is a structural// comparison, so variance annotations are// not in effectconst p: Producer = {    make(): number {        return 42;    }}
```
...
However, this isn’t an instantiation-based comparison, because the object literal is an anonymous type, not a `Producer`.
...
```
ts// Error, this interface is definitely contravariant on Tinterface Foo {  consume: (arg: T) => void;}
```
