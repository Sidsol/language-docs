---
sourceUrl: "https://www.typescriptlang.org/docs/handbook/mixins.html"
fetchedAt: "2026-04-19T23:34:52.309Z"
language: "TypeScript"
title: "Mixins"
---
# Mixins
...
```
tsclass Sprite {  name = "";  x = 0;  y = 0;   constructor(name: string) {    this.name = name;  }}Try
```
...
ts// Compose a new class from the Sprite class,// with the Mixin Scale applier:const .Scaling;
    ...
    prototype: Scale.Scaling;
} & typeof Sprite' >Scale(Sprite); const .Scaling & Sprite' >flappySprite = new  Scale.Scaling & Sprite' >EightBitSprite("Bird");.Scaling & Sprite' >flappySprite..setScale(scale: number): void' >setScale(0.8);console.log(.Scaling & Sprite' >flappySprite..scale: number' >scale);Try
```
...
```
ts// This was our previous constructor:type  {}' >Constructor = new (...args: any[]) => {};// Now we use a generic version which can apply a constraint on// the class which this mixin is applied totype  = new (...args: any[]) => T' >GConstructorT = {}> = new (...args: any[]) => ' >T;Try
```
...
}' >Positionable =  = new (...args: any[]) => T' >GConstructorsetPos: (x: number, y: number) => void }>;type  Sprite' >Spritable =  = new (...args: any[]) => T' >GConstructor;type  {
    print: () => void;
}' >Loggable =  = new (...args: any[]) => T' >GConstructorprint: () => void }>;Try
```
...
```
tsfunction (Base: TBase): {
    new (...args: any[]): Jumpable;
    prototype: Jumpable.Jumpable;
} & TBase' >Jumpable.Jumpable;
} & TBase' >TBase extends  {
    setPos: (x: number, y: number) => void;
...
```
tsfunction (): typeof Base' >baseT>() {  class Base {    static prop: (): typeof Base' >T;  }  return Base;} function (): typeof Derived' >derivedT>() {  class Derived extends (): typeof Base' >baseT>() {    static anotherProp: (): typeof Derived' >T;  }  return Derived;} class Spec extends (): typeof Derived' >derived() {} Spec..prop: string' >prop; // stringSpec..anotherProp: string' >anotherProp; // stringTry
```