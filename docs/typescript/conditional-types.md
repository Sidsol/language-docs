...
fetchedAt: "2026-04-19T23:34:51.675Z"
language: "TypeScript"
...
```
tsinterface Animal {  live(): void;}interface Dog extends Animal {  woof(): void;} type Example1 = Dog extends Animal ? number : string;        type Example1 = number type Example2 = RegExp extends Animal ? number : string;        type Example2 = stringTry
```
...
```
tstype  = T extends number ? IdLabel : NameLabel' >NameOrIdT extends number | string> = ' >T extends number  ? IdLabel  : NameLabel;Try
```
...
```
tsfunction (idOrName: T): NameOrId' >createLabel' >T extends number | string>(idOrName: (idOrName: T): NameOrId' >T):  = T extends number ? IdLabel : NameLabel' >NameOrId' >T> {  throw "unimplemented";} let a = (idOrName: "typescript"): NameLabel' >createLabel("typescript");   let a: NameLabel let b = (idOrName: 2.8): IdLabel' >createLabel(2.8);   let b: IdLabel let c = (idOrName: "hello" | 42): IdLabel | NameLabel' >createLabel(Math.random() ? "hello" : 42);let c: NameLabel | IdLabelTry
```
...
```
tstype  = T["message"]' >MessageOfT> = ' >T["message"];Type '"message"' cannot be used to index type 'T'.2536Type '"message"' cannot be used to index type 'T'.Try
```
...
```
tstype  = T["message"]' >MessageOfT extends { message: unknown }> = ' >T["message"]; interface Email {  message: string;} type EmailMessageContents =  = T["message"]' >MessageOf*;              type EmailMessageContents = stringTry
```
...
tstype  = T extends {
    message: unknown;
} ? T["message"] : never' >MessageOfT> = ' >T extends { message: unknown } ? ' >T["message"] : never; interface Email {  message: string;} interface Dog {  bark(): void;} type EmailMessageContents =  = T extends {
    message: unknown;
} ? T["message"] : never' >MessageOf;              type EmailMessageContents = string type DogMessageContents =  = T extends {
    message: unknown;
} ? T["message"] : never' >MessageOf;             type DogMessageContents = neverTry
```
...
```
tstype  = T extends any[] ? T[number] : T' >FlattenT> = ' >T extends any[] ? ' >T[number] : ' >T; // Extracts out the element type.type Str =  = T extends any[] ? T[number] : T' >Flatten;     type Str = string // Leaves the type alone.type Num =  = T extends any[] ? T[number] : T' >Flatten;     type Num = numberTry
```
...
```
tstype  = Type extends (infer Item)[] ? Item : Type' >FlattenType> = ' >Type extends ' >Array ? Item : ' >Type;Try
```
...
```
tstype  = Type extends (...args: never[]) => infer Return ? Return : never' >GetReturnTypeType> = ' >Type extends (...args: never[]) => infer Return  ? Return  : never; type Num =  = Type extends (...args: never[]) => infer Return ? Return : never' >GetReturnType number>;     type Num = number type Str =  = Type extends (...args: never[]) => infer Return ? Return : never' >GetReturnType string>;     type Str = string type Bools =  = Type extends (...args: never[]) => infer Return ? Return : never' >GetReturnType boolean[]>;      type Bools = boolean[]Try
```
...
```
tsdeclare function stringOrNum(x: string): number;declare function stringOrNum(x: number): string;declare function stringOrNum(x: string | number): string | number; type T1 =  any> = T extends (...args: any) => infer R ? R : any' >ReturnType;     type T1 = string | numberTry
```
...
```
tstype  = Type extends any ? Type[] : never' >ToArrayType> = ' >Type extends any ? ' >Type[] : never;Try
```
...
```
tstype  = Type extends any ? Type[] : never' >ToArrayType> = ' >Type extends any ? ' >Type[] : never; type StrArrOrNumArr =  = Type extends any ? Type[] : never' >ToArray;           type StrArrOrNumArr = string[] | number[]Try
```
...
```
ts   = Type extends any ? Type[] : never' >ToArray |  = Type extends any ? Type[] : never' >ToArray;Try
```
...
```
tstype  = [Type] extends [any] ? Type[] : never' >ToArrayNonDistType> = [' >Type] extends [any] ? ' >Type[] : never; // &apos;ArrOfStrOrNum&apos; is no longer a union.type ArrOfStrOrNum =  = [Type] extends [any] ? Type[] : never' >ToArrayNonDist;          type ArrOfStrOrNum = (string | number)[]Try
```