...
fetchedAt: "2026-04-19T23:34:51.068Z"
language: "TypeScript"
...
```
tsfunction  void): void' >greeter( void' >fn: (a: string) => void) {   void' >fn("Hello, World");} function printToConsole(s: string) {  console.log(s);}  void): void' >greeter(printToConsole);Try
```
...
```
tstype  void' >GreetFunction = (a: string) => void;function greeter(fn:  void' >GreetFunction) {  // ...}Try
```
...
```
tstype DescribableFunction = {  description: string;  (someArg: number): boolean;};function doSomething(fn: DescribableFunction) {  console.log(fn.description + " returned " +  boolean' >fn(6));} function myFunc(someArg: number) {  return someArg > 3;}myFunc.description = "default description"; doSomething(myFunc);Try
```
...
```
tstype  SomeObject' >SomeConstructor = {  new (s: string): SomeObject;};function fn(ctor:  SomeObject' >SomeConstructor) {  return new  SomeObject' >ctor("hello");}Try
```
...
```
tsinterface CallOrConstruct {  (n?: number): string;  new (s: string): Date;} function fn(ctor: CallOrConstruct) {  // Passing an argument of type `number` to `ctor` matches it against  // the first definition in the `CallOrConstruct` interface.  console.log( string' style='border-bottom: solid 2px lightgrey;'>ctor(10));               (parameter) ctor: CallOrConstruct
(n?: number) => string   // Similarly, passing an argument of type `string` to `ctor` matches it  // against the second definition in the `CallOrConstruct` interface.  console.log(new  Date' style='border-bottom: solid 2px lightgrey;'>ctor("10"));                   (parameter) ctor: CallOrConstruct
...
```
tsfunction (arr: Type[]): Type | undefined' >firstElementType>(arr: (arr: Type[]): Type | undefined' >Type[]): (arr: Type[]): Type | undefined' >Type | undefined {  return arr[0];}Try
```
...
```
ts// s is of type &apos;string&apos;const s = (arr: string[]): string | undefined' >firstElement(["a", "b", "c"]);// n is of type &apos;number&apos;const n = (arr: number[]): number | undefined' >firstElement([1, 2, 3]);// u is of type undefinedconst u = (arr: never[]): undefined' >firstElement([]);Try
```
...
```
tsfunction (arr: Input[], func: (arg: Input) => Output): Output[]' >map Output): Output[]' >Input, (arr: Input[], func: (arg: Input) => Output): Output[]' >Output>(arr: (arr: Input[], func: (arg: Input) => Output): Output[]' >Input[],  Output' >func: (arg: (arr: Input[], func: (arg: Input) => Output): Output[]' >Input) => (arr: Input[], func: (arg: Input) => Output): Output[]' >Output): (arr: Input[], func: (arg: Input) => Output): Output[]' >Output[] {  return arr..map(callbackfn: (value: Input, index: number, array: Input[]) => Output, thisArg?: any): Output[]' >map( Output' >func);} // Parameter &apos;n&apos; is of type &apos;string&apos;// &apos;parsed&apos; is of type &apos;number[]&apos;const parsed = (arr: string[], func: (arg: string) => number): number[]' >map(["1", "2", "3"], (n) => parseInt(n));Try
```
...
```
tsfunction (a: Type, b: Type): Type' >longestType extends { length: number }>(a: (a: Type, b: Type): Type' >Type, b: (a: Type, b: Type): Type' >Type) {  if (a.length >= b.length) {    return a;  } else {    return b;  }} // longerArray is of type &apos;number[]&apos;const longerArray = (a: number[], b: number[]): number[]' >longest([1, 2], [1, 2, 3]);// longerString is of type &apos;alice&apos; | &apos;bob&apos;const longerString = (a: "alice" | "bob", b: "alice" | "bob"): "alice" | "bob"' >longest("alice", "bob");// Error! Numbers don&apos;t have a &apos;length&apos; propertyconst notOK = (a: {
    length: number;
}, b: {
    length: number;
}): {
    length: number;
}' >longest(10, 100);Argument of type 'number' is not assignable to parameter of type '{ length: number; }'.2345Argument of type 'number' is not assignable to parameter of type '{ length: number; }'.Try
```
...
```
tsfunction (obj: Type, minimum: number): Type' >minimumLengthType extends { length: number }>(  obj: (obj: Type, minimum: number): Type' >Type,  minimum: number): (obj: Type, minimum: number): Type' >Type {  if (obj.length >= minimum) {    return obj;  } else {    return { length: minimum };Type '{ length: number; }' is not assignable to type 'Type'.
...
```
ts// &apos;arr&apos; gets value { length: 6 }const arr = (obj: number[], minimum: number): number[]' >minimumLength([1, 2, 3], 6);// and crashes here because arrays have// a &apos;slice&apos; method, but not the returned object!console.log(arr..slice(start?: number, end?: number): number[]' >slice(0));Try
```
...
```
tsfunction (arr1: Type[], arr2: Type[]): Type[]' >combineType>(arr1: (arr1: Type[], arr2: Type[]): Type[]' >Type[], arr2: (arr1: Type[], arr2: Type[]): Type[]' >Type[]): (arr1: Type[], arr2: Type[]): Type[]' >Type[] {  return arr1..concat(...items: ConcatArray[]): Type[] (+1 overload)' >concat(arr2);}Try
```
...
```
tsconst arr = (arr1: number[], arr2: number[]): number[]' >combine([1, 2, 3], ["hello"]);Type 'string' is not assignable to type 'number'.2322Type 'string' is not assignable to type 'number'.Try
```
...
```
tsconst arr = (arr1: (string | number)[], arr2: (string | number)[]): (string | number)[]' >combine([1, 2, 3], ["hello"]);Try
```
...
```
tsfunction (arr: Type[]): Type' >firstElement1Type>(arr: (arr: Type[]): Type' >Type[]) {  return arr[0];} function (arr: Type): any' >firstElement2Type extends any[]>(arr: (arr: Type): any' >Type) {  return arr[0];} // a: number (good)const a = (arr: number[]): number' >firstElement1([1, 2, 3]);// b: any (bad)const b = (arr: number[]): any' >firstElement2([1, 2, 3]);Try
```
...
```
tsfunction (arr: Type[], func: (arg: Type) => boolean): Type[]' >filter1 boolean): Type[]' >Type>(arr: (arr: Type[], func: (arg: Type) => boolean): Type[]' >Type[],  boolean' >func: (arg: (arr: Type[], func: (arg: Type) => boolean): Type[]' >Type) => boolean): (arr: Type[], func: (arg: Type) => boolean): Type[]' >Type[] {  return arr..filter(predicate: (value: Type, index: number, array: Type[]) => unknown, thisArg?: any): Type[] (+1 overload)' >filter( boolean' >func);} function  boolean>(arr: Type[], func: Func): Type[]' >filter2(arr: Type[], func: Func): Type[]' >Type,  boolean>(arr: Type[], func: Func): Type[]' >Func extends (arg:  boolean>(arr: Type[], func: Func): Type[]' >Type) => boolean>(  arr:  boolean>(arr: Type[], func: Func): Type[]' >Type[],   boolean' >func:  boolean>(arr: Type[], func: Func): Type[]' >Func):  boolean>(arr: Type[], func: Func): Type[]' >Type[] {  return arr..filter(predicate: (value: Type, index: number, array: Type[]) => unknown, thisArg?: any): Type[] (+1 overload)' >filter( boolean' >func);}Try
```
...
```
tsfunction (s: Str): void' >greetStr extends string>(s: (s: Str): void' >Str) {  console.log("Hello, " + s);} (s: "world"): void' >greet("world");Try
```
...
```
tsfunction  void): void' >myForEach(arr: any[],  void' >callback: (arg: any, index?: number) => void) {  for (let i = 0; i length; i++) {     void' >callback(arr[i], i);  }}Try
```
...
```
ts void): void' >myForEach([1, 2, 3], (a) => console.log(a)); void): void' >myForEach([1, 2, 3], (a, i) => console.log(a, i));Try
```
...
```
tsfunction  void): void' >myForEach(arr: any[],  void' >callback: (arg: any, index?: number) => void) {  for (let i = 0; i length; i++) {    // I don&apos;t feel like providing the index today     void' >callback(arr[i]);  }}Try
```
...
```
ts void): void' >myForEach([1, 2, 3], (a, i) => {  console.log(i.toFixed());'i' is possibly 'undefined'.18048'i' is possibly 'undefined'.});Try
```
...
```
tsconst  void;
}' >user = {  id: 123,   admin: false,   void' >becomeAdmin: function () {    this.admin = true;  },};Try
```
...
```
tsinterface DB {   boolean): User[]' >filterUsers( boolean' >filter: (this: User) => boolean): User[];} const db =  DB' >getDB();const admins = db. boolean): User[]' >filterUsers(function (this: User) {  return this.admin;});Try
```
...
```
tsinterface DB {   boolean): User[]' >filterUsers( boolean' >filter: (this: User) => boolean): User[];} const db =  DB' >getDB();const admins = db. boolean): User[]' >filterUsers(() => this.admin);The containing arrow function captures the global value of 'this'.Element implicitly has an 'any' type because type 'typeof globalThis' has no index signature.70417017The containing arrow function captures the global value of 'this'.Element implicitly has an 'any' type because type 'typeof globalThis' has no index signature.Try
```
...
```
tsfunction safeParse(s: string): unknown {  return JSON. any): any' >parse(s);} // Need to be careful with &apos;obj&apos;!const obj = safeParse(someRandomString);Try
```
...
```
tsfunction fail(msg: string): never {  throw new  Error' >Error(msg);}Try
```
...
```
tsfunction multiply(n: number, ...m: number[]) {  return m..map(callbackfn: (value: number, index: number, array: number[]) => number, thisArg?: any): number[]' >map((x) => n * x);}// &apos;a&apos; gets value [10, 20, 30, 40]const a = multiply(10, 1, 2, 3, 4);Try
```


In TypeScript, the type annotation on these parameters is implicitly `any[]` instead of `any`, and any type annotation given must be of the form `Array` or `T[]`, or a tuple type (which we’ll learn about later).
...
```
tsconst arr1 = [1, 2, 3];const arr2 = [4, 5, 6];arr1..push(...items: number[]): number' >push(...arr2);Try
```
...
```
tstype  void' >voidFunc = () => void; const f1:  void' >voidFunc = () => {  return true;}; const f2:  void' >voidFunc = () => true; const f3:  void' >voidFunc = function () {  return true;};Try
```
...
```
tsconst v1 =  void' >f1(); const v2 =  void' >f2(); const v3 =  void' >f3();Try
```
...
```
tsconst src = [1, 2, 3];const dst = [0]; src..forEach(callbackfn: (value: number, index: number, array: number[]) => void, thisArg?: any): void' >forEach((el) => dst..push(...items: number[]): number' >push(el));Try
```
...
```
tsfunction f2(): void {  // @ts-expect-error  return true;} const  void' >f3 = function (): void {  // @ts-expect-error  return true;};Try
```
