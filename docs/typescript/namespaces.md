---
sourceUrl: "https://www.typescriptlang.org/docs/handbook/namespaces.html"
fetchedAt: "2026-04-19T23:34:52.444Z"
language: "TypeScript"
title: "Namespaces"
---
# Namespaces
...
```
ts/// namespace Validation {  const lettersRegexp = /^[A-Za-z]+$/;  export class LettersOnlyValidator implements StringValidator {    isAcceptable(s: string) {      return lettersRegexp.test(s);    }  }}
```
...
```
ts/// namespace Validation {  const numberRegexp = /^[0-9]+$/;  export class ZipCodeValidator implements StringValidator {    isAcceptable(s: string) {      return s.length === 5 && numberRegexp.test(s);    }  }}
```
...
If multiple JS files get produced, we’ll need to use `` tags on our webpage to load each emitted file in the appropriate order, for example:
...
Because this library is loaded through a `` tag (instead of a module loader), its declaration uses namespaces to define its shape.
